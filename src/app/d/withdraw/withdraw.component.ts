...
export class WithdrawComponent implements AfterViewInit {
    ...
    withdrawalRequestForm = {
        ...
        validateAndRaiseErrors: () => {
            this.utils.markAllFormControlsAsTouched(this.withdrawalRequestForm.fg);
            if (this.withdrawalRequestForm.fg.invalid) {
        ...
        submit: async () => {
            if (!this.withdrawalRequestForm.validateAndRaiseErrors()) return;
            
            this.withdrawalRequestForm.loading = true;
            
            const data = this.withdrawalRequestForm.fg.value;
            ...
            this.withdrawalRequestForm.loading = false;
            ...
            if (res.minAmountNotMet) {
                ...
                await this.prevWithdrawalRequest.load();
    ...
    //
    // Prev refund request
    prevWithdrawalRequest = {
        loading: false,
        errorLoading: false,
        details: {} as GetPrevWithdrawalTransactionRequestResponse,
        resetWithdrawalRequest: () => {
            this.prevWithdrawalRequest.details.alreadyRequested = false;
            this.prevWithdrawalRequest.details.neverRequested = true;
            this.prevWithdrawalRequest.details.prevRejected = false;
            this.prevWithdrawalRequest.details.prevCompleted = false;
            this.html.initTailwindElements();
        },
        load: async () => {
            this.prevWithdrawalRequest.loading = true;
            ...
            this.prevWithdrawalRequest.loading = false;
            
            if (!res) {
                this.prevWithdrawalRequest.errorLoading = true;
                return;
            ...
            this.prevWithdrawalRequest.details = res;
        }
    }
    ...
    resetRefundAmount() {
        this.withdrawalRequestForm.fg.controls.amount.setValue(0);
    }
}
