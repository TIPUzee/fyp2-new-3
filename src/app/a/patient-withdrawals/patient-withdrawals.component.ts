...
@Component({
    ...
    templateUrl: './patient-withdrawals.component.html',
    styleUrl: './patient-withdrawals.component.scss',
})
export class PatientWithdrawalsComponent implements AfterViewInit {
    //
    // Static variables
    static allObjs: AdminGetPatientWithdrawalsResponse['withdrawals'] = [];
    ...
    static searched = {
        ...
        selectKey: (key: string) => {
            if (PatientWithdrawalsComponent.searched.list.includes(key)) {
                PatientWithdrawalsComponent.searched.list = PatientWithdrawalsComponent.searched.list.filter(k => k !==
                    key);
            } else {
                PatientWithdrawalsComponent.searched.list.push(key);
            }
            PatientWithdrawalsComponent.searched.changeSearch$.next();
        },
        search: (q: string) => {
            PatientWithdrawalsComponent.searched.query = q;
            PatientWithdrawalsComponent.searched.changeSearch$.next();
        },
    };
    //
    ...
    selectedObj: AdminGetPatientWithdrawalsResponse['withdrawals'][0] = {
        amount: 0,
        id: 0,
        patientId: 0,
        receiverEpUsername: '',
    ...
    mainClass = PatientWithdrawalsComponent;
    //
    // View Elements
    ...
    rejectForm = {
        ...
        submit: async () => {
            ...
            const res = await this.http.sendRequest({
                method: 'PUT',
                url: '/a/p/withdrawal/reject',
                jsonData: data,
            }) as AdminRejectPatientWithdrawalRequestResponse | false;
    ...
    completeForm = {
        ...
        submit: async () => {
            ...
            const res = await this.http.sendMultipartRequest({
                method: 'PUT',
                url: '/a/p/withdrawal/complete',
                jsonData: data,
                files: {
                    ss: this.completeForm.fg.controls.ss.value as File
                }
            }) as AdminCompletePatientWithdrawalRequestResponse | false;
    ...
    constructor(
        ...
        PatientWithdrawalsComponent.searched.changeSearch$.pipe(takeUntilDestroyed()).subscribe(() => {
            let list = PatientWithdrawalsComponent.searched.list.map(l => l);
            if (list.length === 0) {
            ...
            this.html.dataTableSearch(
                ...
                PatientWithdrawalsComponent.searched.query,
                list
            );
        })
    ...
    initDataTable(): void {
        ...
        (
            ...
            const { id } = row;
            this.selectedObj = PatientWithdrawalsComponent.allObjs.find(p => p.id === id) || this.selectedObj;
    ...
    async load({ id, force }: { id?: number, force?: boolean } = {}) {
        if (PatientWithdrawalsComponent.allObjs.length && !id && !force) {
            this.change$.next();
            return;
        }
        
        if (PatientWithdrawalsComponent.loading) return;
        
        PatientWithdrawalsComponent.loading = true;
        
        let url = '/a/p/withdrawals';
        if (id) {
            url = `/a/p/withdrawal/${ id }`;
        }
        
        const res = await this.http.sendRequest({
            method: 'GET',
            url: url,
        }) as AdminGetPatientWithdrawalsResponse | false;
        
        PatientWithdrawalsComponent.loading = false;
        ...
        if (id) {
            if (res.withdrawals.length === 0) {
                PatientWithdrawalsComponent.allObjs = PatientWithdrawalsComponent.allObjs.filter(p => p.id !== id);
            } else {
                const withdrawals = res.withdrawals[0];
                const index = PatientWithdrawalsComponent.allObjs.findIndex(p => p.id === id);
                if (index !== -1) {
                    PatientWithdrawalsComponent.allObjs[index] = withdrawals;
                } else {
                    PatientWithdrawalsComponent.allObjs.push(withdrawals);
                }
            }
        } else {
            PatientWithdrawalsComponent.allObjs = res.withdrawals;
        }
        
        // also update the selected object
        this.selectedObj.id = -1;
        this.selectedObj = PatientWithdrawalsComponent.allObjs.find(p => p.id === this.selectedObj.id) ||
            this.selectedObj;
        if (this.selectedObj.id === -1) {
            this.selectedObj = PatientWithdrawalsComponent.allObjs[0] || this.selectedObj;
            this.possibleActionsModal.close();
        }
        this.change$.next();
    }
    ...
    searchByUrlQueryParam() {
        ...
        for (const col of this.columns) {
            if (urlParams.has(col.field)) {
                PatientWithdrawalsComponent.searched.list = [col.field];
                PatientWithdrawalsComponent.searched.query = urlParams.get(col.field) || '';
                PatientWithdrawalsComponent.searched.changeSearch$.next();
                break;
            }
        }
    ...
    updateDataTable() {
        ...
        const rows = PatientWithdrawalsComponent.allObjs.map(w => [
            w.id,
            w.patientId,
            w.status,
